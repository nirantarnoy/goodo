...
use backend\models\TranstableSearch;
...
/**
 * TransadjustController implements the CRUD actions for Transadjust model.
 */
class TranstableController extends Controller
{
    ...
    /**
     * Lists all Transadjust models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new TranstableSearch();
    ...
    /**
     * Displays a single Transadjust model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new Transadjust model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate()
    {
        $model = new \backend\Models\Transtable();
        $modelrunno = \backend\Models\Sequence::find()->where(['activitytype'=>8])->one();
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if($model->save()){
                 ...
                 $model2 = \backend\Models\Sequence::find()->where(['activitytype'=>8])->one();
    ...
    /**
     * Updates an existing Transadjust model.
    ...
    /**
     * Deletes an existing Transadjust model.
...
\backend\Models\Transline::deleteAll(['transtableid'=>$id]);
    ...
    /**
     * Finds the Transadjust model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Transadjust the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = \backend\Models\Transtable::findOne($id)) !== null) {
            return $model;
        } else {
...
//           echo "org_group=". count($org_group);
    ...
    public function AddOnhand($prodid,$wh,$loc,$lot,$qty){
        ...
        if(count($model)>0){
            ...
            $model2 = new \backend\Models\Productonhand();
            $model2->prodid = $prodid;
            $model2->warehouseid = $wh;
            $model2->locationid = $loc;
            $model2->lotid = $lot;
            $model2->realqty = $qty;
            $model2->qty = $qty;
